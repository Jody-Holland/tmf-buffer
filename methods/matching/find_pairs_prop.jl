...
const THRESHOLDS = Float32[100.0, 10.0, 15.0, 0.05, 0.05, 0.05, 0.05, 0.05, 0.05]
...
function find_proportion_iteration(
    ...
    for k_idx in 1:n_k
        ...
        # Per-pixel local propensity model: this K pixel vs its candidate M pixels
        cand_rows = @view m_local_x[cand_idx, :]
        k_row = @view k_local_x[k_idx, :]
        weights = local_propensity_weights(k_row, cand_rows)
...
function _iteration_tuple(args)
    (msf, start_year, evaluation_year, out, kf, iseed, sseed, min_pm, max_pm) = args
    return find_proportion_iteration(msf, start_year, evaluation_year, out, kf, iseed, sseed, min_pm, max_pm)
end
    ...
    try
        ...
        for b in 1:total_batches
            ...
            if !isempty(worker_ids)
                batch_args = [
                    (msf, start_year, evaluation_year, output_folder, kf, iseed, sseed, min_potential_matches, max_potential_matches)
